  ...
  execute: async (client, message) => {
    ...
    if (userdata) {
      if (userdata.coins < amount) {
        ...
        message.channel.send('Minimum roulette amount is 0 coins!');
        return;
      }
      ...
      if (win > 0) {
        const rewardMessage = new MessageEmbed()
          ...
          .addField('You won!', `You won ${(amount + amount * win).toFixed(2)} coins`)
          .addField('Credits', `You have ${(userdata.coins + (amount * win)).toFixed(2)} coins`);
        ...
        const rewardMessage = new MessageEmbed()
          ...
          .setColor('#ff8888')
          .addField('Number Drawn', `${draw}`)
          .addField('You lost!', `You lost ${amount} coins`)
          .addField('Credits', `You have ${userdata.coins - amount} coins`);
        message.channel.send(rewardMessage);
